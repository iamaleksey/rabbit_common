...
-type(amqp_method_field_name() ::
       ( prefetch_count | reply_code | exchange | global
       | delivery_tag | routing_key | arguments | nowait
       | nowait | integer_op | ticket | integer_1
       | requeue | queue | mechanisms | auto_delete
       | realm | queue | nowait | reply_code
       | ticket | identifier | ticket | prefetch_size
       | arguments | frame_max | read | ticket
       | reply_text | durable | consumer_tag | ticket
       | exchange | global | ticket | passive
       | routing_key | integer_result | mandatory | host
       | auto_delete | routing_key | mandatory | nowait
       | exchange | delivery_tag | prefetch_count | message_count
       | virtual_host | consumer_tag | global | consumer_tag
       | reply_text | routing_key | operation | reply_text
       | active | reply_code | immediate | version_minor
       | integer_2 | content_checksum | nowait | reply_text
       | known_hosts | message_count | result | multiple
       | known_hosts | heartbeat | consumer_tag | routing_key
       | consumer_tag | exclusive | routing_key | string_result
       | internal | queue | consumer_tag | exchange
       | reply_code | queue | channel_max | queue
       | ticket | routing_key | class_id | ticket
       | exchange | consumer_tag | requeue | exchange
       | consumer_tag | queue | string_op | client_properties
       | redelivered | durable | if_unused | no_ack
       | no_local | nowait | prefetch_size | nowait
       | nowait | message_count | ticket | delivery_tag
       | meta_data | active | queue | string_2
       | method_id | prefetch_count | no_local | challenge
       | passive | out_of_band | reply_code | reply_code
       | consumer_tag | dtx_identifier | if_empty | nowait
       | exchange | no_local | routing_key | locale
       | integer_3 | arguments | result | consumer_tag
       | message_count | response | ticket | delivery_tag
       | queue | mechanism | consumer_tag | delivery_tag
       | exclusive | reply_text | immediate | consumer_count
       | queue | consumer_tag | consume_rate | exclusive
       | insist | requeue | exclusive | integer_4
       | capabilities | channel_max | staged_size | no_ack
       | exchange | immediate | type | delivery_tag
       | ticket | frame_max | exchange | ticket
       | exchange | nowait | exchange | redelivered
       | routing_key | consumer_tag | details | heartbeat
       | routing_key | table | version_major | cluster_id
       | ticket | class_id | write | identifier
       | prefetch_size | ticket | ticket | locales
       | reply_text | arguments | routing_key | requeue
       | nowait | exclusive | no_ack | content_size
       | passive | method_id | delivery_tag | if_unused
       | redelivered | string_1 | operation | delivery_tag
       | mandatory | response | exchange | server_properties
       | active | exchange | consumer_tag | identifier
       | exchange | nowait | consumer_tag | queue
       | multiple | queue )).
